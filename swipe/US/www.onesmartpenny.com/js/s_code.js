...
if(document.location.hostname.indexOf('stg') > -1) { s_account = "escdevdev" }
...
if(document.location.href.indexOf('/cac/') > -1) { s.queryVarsList = s.queryVarsList + ',code' }
if(document.location.href.indexOf('/test/') > -1) { s.queryVarsList = s.queryVarsList + ',section' } // for stg1 only!

/* Form Analysis Config */
s.formList='search,search2,seek1,jumpy3,clientAccessForm,general-form_disable-form-success,searchform,MasterPage,CDFRegistrationForm,CAPIDRegistrationForm,disputeForm,RegistrationForm,OrderSummaryBegin,submitform,searchformTop,outWallet,credEdForm,beginForm1,logoutForm';
...
function s_doPlugins(s) {
	...
	if(/expReferrer/.test(document.location.search)) {
		if(s.getQueryParam('expReferrer').length) {
			s.referrer = s.getQueryParam('expReferrer');
		} else {
	...
	/* populate eVar2/prop2 based on query string parameter "intcmp", only populate once */
	if(!s.eVar2){s.eVar2=s.getQueryParam('intcmp')}
	s.eVar2=s.getValOnce(s.eVar2,"s_intcmp",0)
	s.prop2=s.eVar2

	/* populate eVar57/prop57 based on query string parameter "dyncmp" */
	if(!s.eVar57){s.eVar57=s.getQueryParam('dyncmp')}
	s.prop57=s.eVar57
	...
	if(s.pageType=="errorPage"){ s.pageName="404:"+document.location; }
	...
	/* Eloqua Campaign Tracking */
	/* Eloqua Message ID*/
	if (!s.eVar20) {
		s.eVar20 = s.getQueryParam('SP_MID'); // Set eVar20 with Eloqua Mailing ID
		s.eVar20 = s.getValOnce(s.eVar20,'s_var_20',0);
	}
	
	/* Eloqua Recipient ID */
	if (!s.eVar21) {
		s.eVar21 = s.getQueryParam('SP_RID'); // Set eVar21 with Eloqua Recipient ID
